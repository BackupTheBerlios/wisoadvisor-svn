...
class ucRegistration extends UseCase
{
	...
	/**
	 * checkRegistration() f�hrt die Registrierung durch
	 */
	private function checkRegistration()
	{
		...
		//aus den �bergebenen Daten wird ein neues UserObjekt gebaut:
		...
		//zuerst checken: ist die eMailadresse g�ltig?
		...
		//stimmen Passwort und -Wiederholung �berein?
...
//			$confirmed = 'false';
//			IM MOMENT WIRD JEDER USER 'SOFORT' FREIGESCHALTET, DAMIT EINE FREIE REGISTRIERUNG OHNE EMAILBEST�TIGUNG M�GLICH IST!
			$confirmed = true;
			
			//UserObjekt weiter bef�llen:
...
//				//2. Best�tigungsseite anzeigen
//				$this->showConfirmation($email);
//				IM MOMENT - SIEHE OBEN - WIRD KEINE BEST�TIGUNG ANGEZEIGT, SONDERN DIREKT WIEDER DER LOGIN!
//				header('location:'.$this->getUseCaseLink('login', '', Array('target='.urlencode($this->getParam()->getParameter('target')))));
//				$this->setOutputType(USECASE_NOTYPE);
				
				//jetzt setzen wir einfach noch die ben�tigten Parameter und loggen den User automatisch ein:
	...
	/**
	 * checkConfirmation() �berpr�ft, ob die Registrierung freigeschaltet werden kann
	...
	/**
	 * showVerificationConfirmation() zeigt die "Verifizierungsbest�tigungsseite" an
	 * @deprecated derzeit nicht verwendet
	 */	
	...
	private function showRegistrationForm($user = null, $error = '')
	{
		...
		//die gender-Radio-Boxen m�ssen extra bereitgestellt werden:
	...
	/**
	 * showConfirmation() zeigt die Best�tigungsseite nach erfolgreicher Registrierung an
	...
	/**
	 * generateConfirmationMail() generiert eine eMail mit dem Best�tigungslink zur Freischaltung der Registrierung
	 * @param $email eMailadresse des Empf�ngers
	 * @param $username Name des Empf�ngers
...
//		$link = 'http://'.$_SERVER['SERVER_NAME'].$this->getOwnLink('confirm').'&email='.$email.'&authentication='.$authCode;
		...
		//damit k�nnen alle Teile der eMail erzeugt werden:
	...
	/**
	 * generateVerificationString() erzeugt einen zuf�lligen String zur Verification der eMailadresse
	 * @return ein String mit dem Zufallscode
	 */
	 private function generateVerificationString()
	 {
	 	//einige "feste" Definitionen:
	 	$rndPossibilities['low'] = 48; //untere Grenze f�r den Ascii-Wert
	 	$rndPossibilities['up'] = 122; //obere Grenze f�r den Ascii-Wert
	 	$rndPossibilities['interval_numbers']['low'] = 48; //untere Grenze f�r Ziffern
	 	$rndPossibilities['interval_numbers']['up'] = 57; //obere Grenze f�r Ziffern
	 	$rndPossibilities['interval_chars_big']['low'] = 65; //untere Grenze f�r Gro�buchstaben
	 	$rndPossibilities['interval_chars_big']['up'] = 90; //obere Grenze f�r Gro�buchstaben
	 	$rndPossibilities['interval_chars_small']['low'] = 97; //untere Grenze f�r Kleinbuchstaben
	 	$rndPossibilities['interval_chars_small']['up'] = 122; //obere Grenze f�r Kleinbuchstaben
	 	
	 	//L�nge des Verfifikationsstrings:
		...
		for ($i = 0; $i <= ($length - 1); $i++)
		{
	 		...
	 		//h�nge das ermittelte Ascii-Zeichen an den String an
	 		$code .= chr($rndNumber);
		}
