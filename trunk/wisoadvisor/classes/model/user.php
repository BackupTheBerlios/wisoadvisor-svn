...
/** 
 * Die Klasse User repr�sentiert jeden Benutzer, der sich am System anmelden kann.
 * 
...
class User extends ModelHelper {
	 ...
	 * @param int $uid User-ID, die das gew�nschte Objekt identifiert.
	 * @return User Das gew�nschte Objekt oder null, falls kein Objekt mit diser ID existiert.
	 */
		...
		if ($this->id==User::ID_NEW)
		{
			$result = $context->getDb()->preparedQuery($context->getConf()->getConfString('sql', 'user', 'storeInsert'), 
														Array($this->userName, 
														      ...
														      $this->matnr, 
														      $this->studies));
			//zus�tzlich ggf. die "richtige" ID aus der DB gleich setzen:
			if ($result) $this->setId( $context->getDb()->lastId() );
			else throw new ModelException("User::storeInDb: Fehler beim Einf�gen in die Datenbank:<br>".$context->getDb()->getError(), 0);
		}
		else
		{
