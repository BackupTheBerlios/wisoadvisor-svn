<?php
//WisoadvisorConfiguration enth�lt alle wichtigen Konfigurationsdaten
//als Elternklasse wird Configuration auf dem Framework ben�tigt
...
class WisoadvisorConfiguration extends Configuration
{
	 ...
	 */	
	 public function __construct()
	 {
	 	...
	 	$this->setConfValue('standardusecase', null, null, 'start'); // Der UseCase, der standardm��ig aufgerufen wird
	 	...
	 	$this->setConfValue('standardstep', null, null, 'start'); // Der Schritt, der standardm��ig aufgerufen wird
	 	$this->setConfValue('unauthenticatedExceptions', null, null, Array('login', 'registration', 'static', 'overview', 'info', 'graphics', 'maintenance', 'feedback', 'forgotpassword', 'panorama')); //legt UseCases fest, die auch ohne Login aufgerufen werden d�rfen
	 	
	 	//Datenbankkonfiguration
	 	//ausgelagert in SpecialisedDatabase, damit Serverunabh�ngigkeit erreicht wird!
	 	...
	 	$this->setConfValue('template', 'html', null, 'templates/htmltemplate.tpl'); //Das Template f�r den Seitenaufbau
	 	$this->setConfValue('template', 'popup', null, 'templates/popuptemplate.tpl'); //Ein Template f�r Popup-Fenster
	 	$this->setConfValue('template', 'indicator', 'pre', '###:###'); //Markierung f�r Ersetzungsanweisungen (Prefix)
	 	$this->setConfValue('template', 'indicator', 'after', '###:###'); //Markierung f�r Ersetzungsanweisungen (Postfix)
	 	...
	 	$this->setConfValue('messages', 'pagenotfound', null, 'Die angeforderte Seite wurde nicht gefunden oder kann nur f�r angemeldete Nutzer angezeigt werden. ');
	 	$this->setConfValue('messages', 'surveynotagainallowed', null, 'Der angeforderte Test wurde von Dir bereits ausgef�llt und abgeschickt - eine nochmalige Teilnahme ist nicht m�glich. ');
	 	$this->setConfValue('messages', 'legend_relative', null, '<b>Erkl�rung zu den Grafiken:</b><br/>Die Balken zeigen Dich im Vergleich zu den anderen Testteilnehmern. Der gelbe Strich markiert Dein eigenes Testergebnis, w�hrend die R�nder eines Balkens jeweils das schlechteste bzw. beste Ergebnis der anderen Nutzer (reduziert um die Extremwerte) symbolisieren.<br/><br/><img src="grafik/single_bad.png" class="image_right"/> Du hast leider nur bis zu ein Drittel der gestellten Fragen richtig beantworten k�nnen. Lies Dir die Infoseiten noch einmal gewissenhaft durch und Du wirst den Erfolg sehen, wenn Du den Test wiederholst.<br/><br/><img src="grafik/single_middle.png" class="image_right"/> Du hast zwischen 33,3% und 66,6% aller Fragen richtig beantwortet, damit liegst Du im guten Mittelfeld. Die restlichen Informationen bekommst Du �ber die Informationsseiten und die Homepage der WiSo-Fakult�t.<br/><br/><img src="grafik/single_good.png" class="image_right"/> Gl�ckwunsch, Du hast zwischen 66,6% und 100% der Fragen richtig beantwortet, in diesem Bereich bist Du top informiert!<br/>');
	 	$this->setConfValue('messages', 'legend', null, '<b>Erkl�rung zu den Grafiken:</b><br/>Der Balken ganz oben zeigt Dich im Vergleich zu den anderen Testteilnehmern. Der gelbe Strich markiert Dein eigenes Testergebnis, w�hrend die R�nder des Balkens jeweils das schlechteste bzw. beste Ergebnis der anderen Nutzer (reduziert um die Extremwerte) symbolisieren.<br/>An den Grafiken zu &quot;Deine Ergebnisse im Einzelnen&quot; erkennst Du Deinen Stand im Vergleich zu den Ergebnissen der anderen Testteilnehmer. Der gelbe Strich markiert Dein eigenes Testergebnis, w�hrend der dunkelblaue Bereich die Bandbreite (reduziert um die Extremwerte) der Ergebnisse der anderen Benutzer zeigt.<br/><br/><img src="grafik/single_bad.png" class="image_right"/> Du hast leider nur bis zu ein Drittel der gestellten Fragen richtig beantworten k�nnen. Lies Dir die Infoseiten noch einmal gewissenhaft durch und Du wirst den Erfolg sehen, wenn Du den Test wiederholst.<br/><br/><img src="grafik/single_middle.png" class="image_right"/> Du hast zwischen 33,3% und 66,6% aller Fragen richtig beantwortet, damit liegst Du im guten Mittelfeld. Die restlichen Informationen bekommst Du �ber die Informationsseiten und die Homepage der WiSo-Fakult�t.<br/><br/><br/><img src="grafik/single_good.png" class="image_right"/> Gl�ckwunsch, Du hast zwischen 66,6% und 100% der Fragen richtig beantwortet, in diesem Bereich bist Du top informiert!<br/>');
	 	$this->setConfValue('messages', 'charheader', null, 'Wie gut Du die Fragen beantwortet hast, veranschaulichen die folgenden Grafiken und Texte. Diese sind hinsichtlich verschiedener Gesichtspunkte gegliedert. Dies soll Dir bei der Reflexion Deiner Ergebnisse helfen. Viel Spa� beim Lesen Deiner Auswertung!');
	 	
	 	//Matching von Klassen- auf Dateinamen - hier m�ssen alle ben�tigten Klassen definiert werden
	 	...
	 	//hier unten sollte der �bersicht halber jeder UseCase zumindest seine eigene initialize-Methode bekommen
	...
	/**
	 * configureSql() f�llt die Konfiguration mit den ben�tigten Sql-Abfragen
	 * Hier sind alle Tabellennamen abgelegt; nach M�glichkeit sollten hier alle verwendeten
	 * SQL-Statements (als "prepared Statement") bereits abgelegt werden, so dass sie zentral verf�gbar sind
	 */
	 private function configureSql()
	 {
	 	//Tabellennamen f�r die Datenbank
	 	$tablePrefix = 'advisor__';
	 	$loggingTablePrefix = $tablePrefix.'log_';
	 	
	 	//Allgemeine Tabellen
		$table['user'] = $tablePrefix.'user'; //speichert die Nutzerdaten
		$table['targetgroups'] = $tablePrefix.'targetgroups'; // enth�lt die unterschiedlichen Zielgruppen
		$table['textelements'] = $tablePrefix.'textelements'; // enth�lt Textbausteine
		$table['te_typ'] = $tablePrefix.'textelement_types'; // Typisiert (=gruppiert) die Textbausteine
		$table['tg_te'] = $tablePrefix.'targetgroups_textelements'; // legt fest, welche Textbausteine f�r welche Zielgruppen geeignet sind
		...
		//Testtool
		$table['answers'] = $tablePrefix.'answers'; // Hier sind m�gliche Antworten abgelegt
		$table['op_res'] = $tablePrefix.'open_results'; // Hier sind Antworten auf OFFENE Fragen abgelegt
		$table['questions'] = $tablePrefix.'questions'; // enth�lt einzelne Fragen
		...
		$table['surveys'] = $tablePrefix.'surveys'; // enth�lt die einzelnen Testmodule
		...
		$table['char'] = $tablePrefix.'characteristics'; // enth�lt die Merkmale (z.B. Einzelkompetenzen), die gepr�ft werden
		...
		$table['questionblocks'] = $tablePrefix.'questionblocks'; // Fragenbl�cke
		$table['open_results'] = $tablePrefix.'open_results'; // Fragenbl�cke
		$table['ratings'] = $tablePrefix.'ratings'; // Ergebnis zu Textelement-Zuordnung
	 	...
	 	$table['infos'] = $tablePrefix.'infos'; // enth�lt die einzelnen Infoseiten und ihre Kurzversionen
	 ...
	 */
	 ...
	 {
		$this->setConfValue('ucStatic', 'templatePath', 'all', 'templates/static/'); //Pfad, in dem die statischen Templates (�ffentliche, die jeder ansehen darf) abgelegt sind
		$this->setConfValue('ucStatic', 'templatePath', 'authenticated', 'templates/static/authenticated/'); //Pfad, in dem die statischen Templates (nur f�r eingeloggte Nutzer) abgelegt sind
		$this->setConfValue('ucStatic', 'suffix', null, '.tpl'); //Datei-Suffix der Templates
	 }

	/**
	 * configureUcLogin() f�llt die Konfiguration mit den ben�tigten Daten f�r ucLogin
	 */
	 private function configureUcLogin()
	 {
		$this->setConfValue('ucLogin', 'loginform_tpl', null, 'templates/ucLogin/loginform.tpl'); //Template f�r das Loginfenster
		$this->setConfValue('ucLogin', 'loginbox_tpl', null, 'templates/ucLogin/loginbox.tpl'); //Template f�r die kleine Loginbox
	 ...
	 {
		$this->setConfValue('ucSession', 'authenticated_tpl', null, 'templates/ucSession/authenticated.tpl'); //Template f�r authentifizierte Benutzer
		//Variable Teile im Template:
	...
	/**
	 * configureUcChangeUserData() f�llt die Konfiguration mit den ben�tigten Daten
	 */
	 private function configureUcChangeUserData()
	 {
		$this->setConfValue('ucChangeUserData', 'changeform_tpl', null, 'templates/ucChangeUserData/changeform.tpl'); //Template f�r das Registrierungsfenster
		//Variable Teile im Template:
		...
		$this->setConfValue('ucChangeUserData', 'regex', 'email', "/^(([a-z0-9_-]+(\\.[a-z0-9_-]+)*@([0-9a-z][0-9a-z-]*[0-9a-z]\\.)+([a-z]{2,4}|museum)))$/i"); //g�ltige (i.S. von m�gliche) eMailadresse oder leer
		$this->setConfValue('ucChangeUserData', 'regex', 'password', "/^([\&\�\!\?\=\%\�\�\�\�a-z0-9_-]{5,100})$/i"); //5-100 Zeichen, Alphabeth, Zahlen, teilw. Sonderzeichen
		...
		$this->setConfValue('ucChangeUserData', 'regex', 'username', "/^([ \�\�\�\�a-z0-9_-]{0,100})$/i"); //0-100 Zeichen, Alphabeth, Zahlen, teilw. Sonderzeichen
		//Fehlermeldung(en):
		$this->setConfValue('ucChangeUserData', 'error', 'username', 'Dein Name darf aus h�chstens 100 Zeichen bestehen und darf keine Sonderzeichen enthalten. ');
		$this->setConfValue('ucChangeUserData', 'error', 'email', 'Deine eMailadresse ist ung�ltig. ');
		$this->setConfValue('ucChangeUserData', 'error', 'gender', 'Die Angabe des Geschlechts ist ung�ltig! ');
		$this->setConfValue('ucChangeUserData', 'error', 'birthday', 'Die Geburtstagsangabe ist ung�ltig! ');
		$this->setConfValue('ucChangeUserData', 'error', 'password', 'Dein Passwort muss aus mindestens 5 Zeichen bestehen. ');
		$this->setConfValue('ucChangeUserData', 'error', 'password_repeat', 'Passwort und Passwort-Wiederholung m�ssen �bereinstimmen. ');
		//Nachricht:
		$this->setConfValue('ucChangeUserData', 'message_text', 'stored', 'Wir haben Deine �nderungen erfolgreich gespeichert.');
	 }
	 
	/**
	 * configureUcRegistration() f�llt die Konfiguration mit den ben�tigten Daten f�r ucRegistration
	 */
	 private function configureUcRegistration()
	 {
		$this->setConfValue('ucRegistration', 'registrationform_tpl', null, 'templates/ucRegistration/registrationform.tpl'); //Template f�r das Registrierungsfenster
		$this->setConfValue('ucRegistration', 'confirmation_tpl', null, 'templates/ucRegistration/confirmation.tpl'); //Template f�r die Best�tigungsseite
		$this->setConfValue('ucRegistration', 'error_verification_tpl', null, 'templates/ucRegistration/verification_error.tpl'); //Template f�r die Best�tigungsseite
		$this->setConfValue('ucRegistration', 'confirm_verification_tpl', null, 'templates/ucRegistration/verification_confirmation.tpl'); //Template f�r die Best�tigungsseite
		...
		//regul�re Ausdr�cke zur �berpr�fung der Eingaben: 
		//Hinweis: aufgrund der PHP-Stringbehandlung m�ssen die Regexe in doppelte Anf�hrungszeichen gesetzt werden!
		$this->setConfValue('ucRegistration', 'regex', 'username', "/^([ \�\�\�\�a-z0-9_-]{0,100})$/i"); //0-100 Zeichen, Alphabeth, Zahlen, teilw. Sonderzeichen
		$this->setConfValue('ucRegistration', 'regex', 'email', "/^(([a-z0-9_-]+(\\.[a-z0-9_-]+)*@([0-9a-z][0-9a-z-]*[0-9a-z]\\.)+([a-z]{2,4}|museum)))$/i"); //g�ltige (i.S. von m�gliche) eMailadresse oder leer
		$this->setConfValue('ucRegistration', 'regex', 'password', "/^([\&\�\!\?\=\%\�\�\�\�a-z0-9_-]{5,100})$/i"); //5-100 Zeichen, Alphabeth, Zahlen, teilw. Sonderzeichen
		...
		//Fehlermeldung(en):
		$this->setConfValue('ucRegistration', 'error', 'username', 'Dein Name darf aus h�chstens 100 Zeichen bestehen und darf keine Sonderzeichen enthalten. ');
		$this->setConfValue('ucRegistration', 'error', 'email', 'Bitte gib eine g�ltige eMailadresse ein. ');
		$this->setConfValue('ucRegistration', 'error', 'gender', 'Die Angabe des Geschlechts ist ung�ltig! ');
		$this->setConfValue('ucRegistration', 'error', 'birthday', 'Die Geburtstagsangabe ist ung�ltig! ');
		$this->setConfValue('ucRegistration', 'error', 'password', 'Dein Passwort muss aus mindestens 5 Zeichen bestehen. ');
		$this->setConfValue('ucRegistration', 'error', 'password_repeat', 'Passwort und Passwort-Wiederholung m�ssen �bereinstimmen. ');
		...
		$this->setConfValue('ucRegistration', 'authenticationLength', null, 50); //L�nge des Authentifizierungscode
...
//		$this->setConfValue('ucRegistration', 'email_confirmation', 'subject', 'WiSo@visor-Registrierung best�tigen');
//		$this->setConfValue('ucRegistration', 'email_confirmation', 'template', 'templates/ucRegistration/email.tpl');
		...
		$this->setConfValue('ucRegistration', 'email_confirmation', 'username', 'username'); //Ersetzung f�r den Usernamen
		$this->setConfValue('ucRegistration', 'email_confirmation', 'email', 'email'); //Ersetzung f�r die eMailadresse
		$this->setConfValue('ucRegistration', 'email_confirmation', 'link', 'link'); //Ersetzung f�r den Best�tigungslink
	 }
	 
	/**
	 * configureUcFeedback() f�llt die Konfiguration mit den ben�tigten Daten
	 */
	 private function configureUcFeedback()
	 {
		$this->setConfValue('ucFeedback', 'feedbackform_tpl', null, 'templates/ucFeedback/feedbackform.tpl'); //Template f�r das Formular
		$this->setConfValue('ucFeedback', 'confirmation_tpl', null, 'templates/ucFeedback/confirmation.tpl'); //Template f�r die Best�tigungsseite
		...
		//regul�re Ausdr�cke zur �berpr�fung der Eingaben: 
		//Hinweis: aufgrund der PHP-Stringbehandlung m�ssen die Regexe in doppelte Anf�hrungszeichen gesetzt werden! - deshalb auch immer der doppelte Backslash
		$this->setConfValue('ucFeedback', 'regex', 'email', "/^(([a-z0-9_-]+(\\.[a-z0-9_-]+)*@([0-9a-z][0-9a-z-]*[0-9a-z]\\.)+([a-z]{2,4}|museum)))$/i"); //g�ltige (i.S. von m�gliche) eMailadresse oder leer
		$this->setConfValue('ucFeedback', 'regex', 'subject', "/^([\\:\\.\\&\\�\\ \\!\\?\\=\\%\\�\\�\\�\\�a-z0-9_-]{1,255})$/i"); //1-255 Zeichen, Alphabeth, Zahlen, teilw. Sonderzeichen
		$this->setConfValue('ucFeedback', 'regex', 'sender', "/^([\\:\\.\\&\\�\\!\\ \\?\\=\\%\\�\\�\\�\\�a-z0-9_-]{0,255})$/i"); //0-255 Zeichen, Alphabeth, Zahlen, teilw. Sonderzeichen
		$this->setConfValue('ucFeedback', 'regex', 'reference', "/^([\\:\\.\\&\\�\\!\\?\\=\\%\\�\\�\\�\\�a-z0-9_-]{0,255})$/i"); //0-255 Zeichen, Alphabeth, Zahlen, teilw. Sonderzeichen
		$this->setConfValue('ucFeedback', 'regex', 'message', "/^([\\n\\r\\t\\d\\D\\s\\S\\w\\W\\:\\.\\&\\�\\ \\!\\?\\=\\%\\�\\�\\�\\�a-z0-9_-]+)$/i"); //nicht leer
		
		//Fehlermeldung(en):
		$this->setConfValue('ucFeedback', 'error', 'email', 'Die Angabe Deiner eMailadresse ist freiwillig; wenn Du eine eMailadresse angibst, dann muss diese g�ltig sein. ');
		$this->setConfValue('ucFeedback', 'error', 'sender', 'Die Angabe Deines Namens ist freiwillig; wenn Du Deinen Namen angibst, darf dieser h�chstens 255 Zeichen lang sein. ');
		$this->setConfValue('ucFeedback', 'error', 'subject', 'Der angegebene Betreff ist leer oder enth�lt ung�ltige Zeichen - erlaubt sind nur Buchstaben, Ziffern und Satzzeichen. ');
	...
	/**
	 * configureUcForgotPwd() f�llt die Konfiguration mit den ben�tigten Daten
	 */
	 private function configureUcForgotPwd()
	 {
		$this->setConfValue('ucForgotPwd', 'pwdform_tpl', null, 'templates/ucForgotPwd/pwdform.tpl'); //Template f�r das Formular
		$this->setConfValue('ucForgotPwd', 'confirmation_tpl', null, 'templates/ucForgotPwd/confirmation.tpl'); //Template f�r die Best�tigungsseite
		//Variable Teile in den Templates:
		...
		//regul�re Ausdr�cke zur �berpr�fung der Eingaben: 
		//Hinweis: aufgrund der PHP-Stringbehandlung m�ssen die Regexe in doppelte Anf�hrungszeichen gesetzt werden! - deshalb auch immer der doppelte Backslash
		$this->setConfValue('ucForgotPwd', 'regex', 'email', "/^(([a-z0-9_-]+(\\.[a-z0-9_-]+)*@([0-9a-z][0-9a-z-]*[0-9a-z]\\.)+([a-z]{2,4}|museum)))$/i"); //g�ltige (i.S. von m�gliche) eMailadresse oder leer
		//Fehlermeldung(en):
		$this->setConfValue('ucForgotPwd', 'error', 'email', 'Du hast keine g�ltige eMailadresse angegeben. ');
	...
	/**
	 * configureUcSurvey() f�llt die Konfiguration mit den Daten f�r ucSurvey
	 */
	 private function configureUcSurvey()
	 {
	 	...
	 	$this->setConfValue('ucSurvey', 'startpage', null, 'startpage'); //Den Text f�r die Startseite
	 	$this->setConfValue('ucSurvey', 'startimage', null, 'startimage'); //Das Bild f�r die Startseite
	 	$this->setConfValue('ucSurvey', 'navigation', null, 'navigationpanel'); //Das Navigationspanel
	 	$this->setConfValue('ucSurvey', 'whysurvey', null, 'whysurvey'); //Button zum �ffnen einer Hilfeseite
	 	...
	 	$this->setConfValue('ucSurvey', 'answers', null, 'answers'); //Antwortm�glichkeiten
		...
		//Buttonbeschriftungen: hier sind jeweils die Buttonbeschriftungen definiert
	 	...
	 	$this->setConfValue('ucSurvey', 'button', 'savesurvey', 'Test abschlie�en');
	 	...
	 	$this->setConfValue('ucSurvey', 'button', 'back', '<< Zur�ck');
	 	
	 	$this->setConfValue('ucSurvey', 'replace', 'multiplechoice', '<br/><br/><span class="questionhelp">(Mehrfachantworten m�glich)</span>');
	 ...
	 private function configureUcOverview()
	 {
	 	...
	 	$this->setConfValue('ucOverview', 'text', 'bar_title_best', 'Gl�ckwunsch! Es gibt keinen Kandidaten, der besser ist als Du.');
	...
	private function configureUcGetPdf()
	{
		...
		$this->setConfValue('ucGetPdf', 'email', 'subject', 'WiSo@visor - Deine pers�nlichen Testergebnisse');
